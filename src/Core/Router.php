...
class Router
{
    ...
    private function setRoute($block)
    {
        ...
        $this->klein->respond('POST', $routePath, function()use($param, $blockName, $blockCustom, $method){
            ...
            // Remove $apiToken from params
            if(isset($sendParam['apiToken'])&&strlen($sendParam['apiToken'])>0){
                $apiToken = $sendParam['apiToken'];
                unset($sendParam['apiToken']);
            }else{
                $apiToken = false;
            }
            ...
            // Make request
            $result = $this->httpRequest($vendorUrl, $method, $sendBody, $apiToken);
            echo json_encode($result);
            exit(200);
        });
    ...
    protected function httpRequest($url, $method, $sendBody, $apiToken)
    {
        ...
        try {
            ...
            if($apiToken){
                $clientSetup['headers']['Authorization'] =  'Basic ' . base64_encode($apiToken . ':');
            }
